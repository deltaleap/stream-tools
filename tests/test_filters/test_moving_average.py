...
def test_movave_state_one_arg_two_values() -> None:
    ...
    ) == (b"moving_average(a)", b"1606081071444-0", {b"x": 1.0, b"y": 11.0})
    assert ma_state.update(
        (b"a", b"1606081071587-0", OrderedDict({b"x": b"2.0", b"y": b"19.0"}))
    ) == (b"moving_average(a)", b"1606081071587-0", {b"x": 1.5, b"y": 19.0})
    assert ma_state.update(
        (b"a", b"1606081071619-0", OrderedDict({b"x": b"3.0", b"y": b"21.0"}))
    ) == (b"moving_average(a)", b"1606081071619-0", {b"x": 2.0, b"y": 21.0})
    assert ma_state.update(
        (b"a", b"1606081071991-0", OrderedDict({b"x": b"4.0", b"y": b"31.0"}))
    ) == (b"moving_average(a)", b"1606081071991-0", {b"x": 3.0, b"y": 31.0})
...
def test_movave_state_two_args_two_values() -> None:
    ...
    assert ma_state.update(
        (b"a", b"1606081071587-0", OrderedDict({b"x": b"2.0", b"y": b"19.0"}))
    ) == (b"moving_average(a)", b"1606081071587-0", {b"x": 1.5, b"y": 15.0})
    assert ma_state.update(
        (b"a", b"1606081071619-0", {b"x": b"3.0", b"y": b"21.0"})
    ) == (b"moving_average(a)", b"1606081071619-0", {b"x": 2.0, b"y": 20.0})
    assert ma_state.update(
        (b"a", b"1606081071991-0", {b"x": b"4.0", b"y": b"31.0"})
    ) == (b"moving_average(a)", b"1606081071991-0", {b"x": 3.0, b"y": 26.0})
...
def test_moving_average_init() -> None:
    ...
    ma2 = MovingAverage(stream2, [("x", 5), ("y", 2)])
